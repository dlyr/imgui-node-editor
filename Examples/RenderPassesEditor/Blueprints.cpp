...
struct Node {
    ...
    std::vector<FloatController> Floats;
    ImColor Color;
    NodeType Type;
    ImVec2 Size;
...
static Node *SpawnBlurNode()
{
    s_Nodes.emplace_back(GetNextId(), "Blur", ImColor(255, 128, 200));
    s_Nodes.back().Inputs.emplace_back(GetNextId(), "Input", PinType::Framebuffer);
    s_Nodes.back().Inputs.emplace_back(GetNextId(), "Radius", PinType::Float);
    s_Nodes.back().Outputs.emplace_back(GetNextId(), "Output", PinType::Framebuffer);
...
static Node *SpawnFloatNode()
{
    s_Nodes.emplace_back(GetNextId(), "Float value", ImColor(255, 128, 200));
    s_Nodes.back().Type = NodeType::Float;
    s_Nodes.back().Outputs.emplace_back(GetNextId(), "Output", PinType::Float);
    s_Nodes.back().Floats.push_back({});
    s_Nodes.back().Floats.back().Name  = "test float";
    s_Nodes.back().Floats.back().Min   = 0.001f;
    s_Nodes.back().Floats.back().Max   = 10.f;
    s_Nodes.back().Floats.back().Value = 1.f;
